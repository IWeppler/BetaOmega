...
export function EditBookForm({ bookId }: EditBookFormProps) {
  ...
  useEffect(() => {
    ...
    const loadBook = async () => {
      ...
      if (fetchedBook) {
        setBook(fetchedBook);
        setChapters(fetchedBook.book_content || []);
      }
      setLoading(false);
    };
  ...
  const handleAddChapter = async () => {
    if (!book || !newChapterTitle || !newChapterContent) {
      toast.error(
        "El título y el contenido del capítulo no pueden estar vacíos."
      );
      return;
    }
    ...
    <div className="flex-1 p-6 space-y-6 h-full">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Lista de capítulos existentes. Actualmente hay {chapters.length} de{" "}
            {book.total_chapters}.
          </CardDescription>
        </CardHeader>
        <CardContent>
