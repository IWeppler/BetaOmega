...
import { IBook, IBookContent, ICreateBookContent, IUpdateBookContent } from "@/interfaces";
...
import { createBookContent, deleteContent, updateContent } from "@/services/content.service";
...
export const ChapterManager = ({ book, onChaptersChange }: Props) => {
  ...
  const [editingChapter, setEditingChapter] = useState<IBookContent | null>(null);
  ...
  const createFormik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const newChapterData: ICreateBookContent = {
        book_id: book.id,
        chapter_number: (book.contents?.length || 0) + 1,
        title: values.title,
  ...
  const editFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const result = await updateContent(editingChapter.id, values as IUpdateBookContent);
      if (result) {
            ...
            {book.contents?.map((chapter) => (
              <li key={chapter.id} className="flex items-center justify-between p-3 rounded-lg border bg-muted/20">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* --- MODAL DE EDICIÓN --- */}
                  <Dialog open={editingChapter?.id === chapter.id} onOpenChange={(isOpen) => !isOpen && setEditingChapter(null)}>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="icon" onClick={() => setEditingChapter(chapter)}>
                    ...
                    <DialogContent className="sm:max-w-[625px]">
                      <DialogHeader>
                        <DialogTitle>Editando: Cap. {chapter.chapter_number}</DialogTitle>
                      </DialogHeader>
                      <form onSubmit={editFormik.handleSubmit} className="space-y-4 py-4">
                        <div>
                          <label htmlFor="editTitle" className="block text-sm font-medium mb-1">Título del Capítulo</label>
                          <Input id="editTitle" {...editFormik.getFieldProps("title")} />
                          {editFormik.touched.title && editFormik.errors.title && <p className="text-red-500 text-xs mt-1">{editFormik.errors.title}</p>}
                        </div>
                        <div>
                          <label className="block text-sm font-medium mb-1">Contenido</label>
                          <RichTextEditor
                            content={editFormik.values.md_content}
                            onChange={(newContent) => editFormik.setFieldValue("md_content", newContent)}
                          />
                        </div>
        ...
        <CardContent>
          <form onSubmit={createFormik.handleSubmit} className="space-y-4">
            <div>
              ...
              <Input id="title" {...createFormik.getFieldProps("title")} />
              {createFormik.touched.title && createFormik.errors.title && <p className="text-red-500 text-xs mt-1">{createFormik.errors.title}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Contenido</label>
              <RichTextEditor
                content={createFormik.values.md_content}
                onChange={(newContent) => createFormik.setFieldValue("md_content", newContent)}
              />
              {createFormik.touched.md_content && createFormik.errors.md_content && <p className="text-red-500 text-xs mt-1">{createFormik.errors.md_content}</p>}
            </div>
